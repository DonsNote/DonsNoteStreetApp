...
//MARK: -3.PREVIEW
#Preview {
    UserArtistPageView().environmentObject(Service())
}
        ...
        .overlay (
            HStack(spacing: UIScreen.getWidth(10)){
                if service.userArtist.youtubeURL != "" {
                    Button {
                        UIApplication.shared.open(URL(string: (service.userArtist.youtubeURL)!)!)// TODO: 값 집어넣어야
                ...
                if service.userArtist.instagramURL != "" {
                    Button {
                        UIApplication.shared.open(URL(string: (service.userArtist.instagramURL)!)!)// TODO: 값 집어넣어야
