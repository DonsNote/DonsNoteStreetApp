...
struct MapBuskingModalView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showReport, onDismiss: onDismiss){
            ReportModalView(artistID: viewModel.busking.artistId)
...
extension MapBuskingModalView {
    ...
    var buskingInfoImage: some View {
        CircleBlur(image: viewModel.busking.artistImageURL, width: UIScreen.getWidth(40))
            .overlay {
...
#Preview {
    MapBuskingModalView(viewModel: MapBuskingModalViewModel(busking: Busking()), showPopover: false)
}
